--[=[
	Applies interpreted Figma geometry and styling to Roblox GuiObjects.
	@class Applicator
]=]
...
local function GetShadowExpansionAndOffset(shadow)
	...
	local Expansion = Vector2.new(Radius, Radius) + Vector2.new(Spread, Spread)
...
function Applicator:ApplyChangesFromData(selectedInstance : Instance, data : {[string] : any})
	...
	local ShadowExpansion, ShadowOffset = GetShadowExpansionAndOffset(data.Shadow)
	local CorrectedSize = Size + Vector2.new(Stroke * 2, Stroke * 2) + ShadowExpansion * 2 + ShadowOffset
	local CorrectedPosition = Position - Vector2.new(Stroke, Stroke) - ShadowExpansion + ShadowOffset / 2
